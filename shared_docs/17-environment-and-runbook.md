...
初赛部署不以 Kubernetes 为前提：本地使用 Docker Compose，云端使用 ECS Docker Compose 或云容器服务。Spring Boot 是唯一主业务 API；Python Worker 通过异步任务或内部接口提供采集、解析和模型适配能力，不对外承担主 API 契约。若启用 Playwright，必须记录浏览器运行时依赖、资源成本、超时和静态解析失败时的降级路径。
